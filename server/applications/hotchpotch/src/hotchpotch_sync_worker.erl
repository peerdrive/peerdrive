...
init(Parent, Mode, FromGuid, ToGuid) ->
	...
	case hotchpotch_volman:store(FromGuid) of
		{ok, FromPid} ->
			case hotchpotch_volman:store(ToGuid) of
				{ok, ToPid} ->
					Id = {FromGuid, ToGuid},
					{ok, Monitor} = hotchpotch_hysteresis:start({sync, FromGuid, ToGuid}),
...
loop(State, OldBacklog) ->
	...
	case OldBacklog of
		[] ->
			NewDone = 1,
			Backlog = case hotchpotch_store:sync_get_changes(FromPid, ToGuid) of
				{ok, Value} -> Value;
				Error       -> throw(Error)
			end,
...
loop_check_msg(State, Backlog, Timeout) ->
	...
	receive
		...
		{trigger_rem_store, ToGuid} ->
			hotchpotch_store:sync_finish(FromPid, ToGuid);
...
sync_step({Doc, SeqNum}, S) ->
	...
	case hotchpotch_store:sync_set_anchor(FromPid, ToGuid, SeqNum) of
		ok -> ok;
		Error -> throw(Error)
	end.
...
sync_doc_ff(Doc, From, To) ->
	throws(hotchpotch_broker:sync(Doc, 0, [From, To])).
...
sync_doc_merge(Doc, {FromGuid, _} = From, {ToGuid, _} = To, Strategy) ->
	case hotchpotch_broker:lookup_doc(Doc, [From, To]) of
		{[{_Rev, _}], _PreRevs} ->
			ok;
		{[], []} ->
			ok;
		{Revs, _PreRevs} ->
			case hotchpotch_broker:sync(Doc, 0, [From, To]) of
				{ok, _ErrInfo, _Rev} ->
					ok;
...
latest_strategy(Doc, From, FromRev, FromStat, _To, ToRev, ToStat) ->
	if
		FromStat#rev_stat.mtime >= ToStat#rev_stat.mtime ->
			...
			Handle = throws(hotchpotch_broker:update(Doc, FromRev, keep, [From])),
			try
				throws(hotchpotch_broker:set_parents(Handle, [FromRev, ToRev])),
...
simple_strategy(Doc, From, FromRev, FromStat, To, ToRev, ToStat) ->
	...
	case get_merge_base([FromRev, ToRev], Stores) of
		{ok, BaseRevs} ->
			...
			BaseRev = hd(BaseRevs),
			BaseStat = throws(hotchpotch_broker:stat(BaseRev, Stores)),
...
traverse(Heads, Path, Stores) ->
	lists:foldl(
		fun(Head, {AccHeads, AccPath}) ->
			case hotchpotch_broker:stat(Head, Stores) of
...
merge_hpsd_read(Revs, Stores) ->
	#rev_stat{parts=Parts} = throws(hotchpotch_broker:stat(hd(Revs), Stores)),
...
merge_hpsd_read_loop([Rev|Revs], Stores, FCCs, Acc) ->
	Reader = throws(hotchpotch_broker:peek(Rev, Stores)),
	Data = try
...
merge_hpsd_write(Doc, From, FromRev, ToRev, Type, NewData) ->
	Writer = throws(hotchpotch_broker:update(Doc, FromRev, <<"org.hotchpotch.syncer">>, [From])),
	try
		throws(hotchpotch_broker:set_parents(Writer, [FromRev, ToRev])),
