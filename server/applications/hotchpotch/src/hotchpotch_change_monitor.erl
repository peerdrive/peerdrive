...
-module(hotchpotch_change_monitor).
...
init([]) ->
	...
	hotchpotch_vol_monitor:register_proc(change_monitor),
...
doc_population(Doc) ->
	Stores = hotchpotch_volman:stores(),
	lists:foldl(
		...
		{ok, StorePid} ->
			dict:map(
				fun({Type, Hash}, {StoreSet, PidSet}) ->
					case Type of
						doc ->
							case hotchpotch_store:lookup(StorePid, Hash) of
								{ok, _Rev, _PreRevs} ->
						...
						rev ->
							case hotchpotch_store:contains(StorePid, Hash) of
								true ->
									case sets:size(StoreSet) of
...
fire_trigger(Cause, Type, Hash, Pids) ->
	%io:format("trigger: ~w ~w ~s~n", [Cause, Type, hotchpotch_util:bin_to_hexstr(Hash)]),
	lists:foreach(
