...
start_link() ->
	gen_server:start_link({local, hotchpotch_sync_locks}, ?MODULE, [], []).


%% @doc Locks a document for synchronization. Will block until the lock has
%%      been taken.
lock(Doc) ->
	gen_server:call(hotchpotch_sync_locks, {lock, Doc}, infinity).


%% @doc Unlocks a previously locked document.
unlock(Doc) ->
	gen_server:call(hotchpotch_sync_locks, {unlock, Doc}, infinity).
