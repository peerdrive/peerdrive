...
init(Parent) ->
	hotchpotch_vol_monitor:register_proc(dispatcher),
...
added_store(Store) ->
	lists:foreach(
		fun({Mode, From, To}) -> hotchpotch_synchronizer:sync(Mode, From, To) end,
	...
	lists:foldl(
		fun(RawRule, Acc) ->
			case parse_rule(RawRule) of
				...
				{Mode, Store, To} ->
					case hotchpotch_volman:store(To) of
						{ok, _Pid} -> [{Mode, Store, To} | Acc];
						error      -> Acc
					end;

				{Mode, From, Store} ->
					case hotchpotch_volman:store(From) of
						{ok, _Pid} -> [{Mode, From, Store} | Acc];
						error      -> Acc
					end;
...
load_sync_rules() ->
	{ok, SysStoreGuid} = find_sys_store(hotchpotch_volman:enum()),
	{ok, SysStorePid} = hotchpotch_volman:store(SysStoreGuid),
	{ok, Root} = read_doc(SysStorePid, SysStoreGuid),
	case dict:find(<<"syncrules">>, Root) of
		{ok, {dlink, Doc}} ->
			case read_doc(SysStorePid, Doc) of
				{ok, Rules}      -> Rules;
				{error, _Reason} -> []
			end;
