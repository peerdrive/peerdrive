...
start_link() ->
	supervisor:start_link({local, hotchpotch_store_sup}, ?MODULE, []).
...
spawn_store(Id, Disposition, Module, Arg) ->
	...
	case supervisor:start_child(hotchpotch_store_sup, ChildSpec) of
		{error, already_present} ->
			supervisor:restart_child(hotchpotch_store_sup, Id);
...
stop_store(Id) ->
	case supervisor:terminate_child(hotchpotch_store_sup, Id) of
		ok                 -> ok;
		{error, not_found} -> ok;
		Error              -> Error
	end.
