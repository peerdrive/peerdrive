...
-module(hotchpotch_ifc_vfs_fuse).
-ifdef(have_fuserl).
%-behaviour(fuserl).
...
releasedir(_, _Ino, #fuse_file_info{fh=Id}, _Cont, S) ->
	% no need to call hotchpotch_ifc_vfs_fuse:close() yet...
...
call_vfs(Fun, Args, #state{vfs_state=VfsState} = S) ->
	{OkErr, Reply, NewVfsState} = (catch apply(hotchpotch_ifc_vfs_common,
		Fun, Args ++ [VfsState])),
...
call_vfs_handle(Fun, Args, FuseHandle, S) ->
	...
	case catch apply(hotchpotch_ifc_vfs_common, Fun, [VfsState, VfsHandle | Args]) of
		{OkErr, Reply} ->
			{OkErr, Reply, S};
