...
start_link() ->
	gen_event:start_link({local, vol_monitor}).


trigger_add_store(StoreGuid) ->
	gen_event:notify(vol_monitor, {trigger_add_store, StoreGuid}).

trigger_rem_store(StoreGuid) ->
	gen_event:notify(vol_monitor, {trigger_rem_store, StoreGuid}).

trigger_add_rev(StoreGuid, Rev) ->
	gen_event:notify(vol_monitor, {trigger_add_rev, StoreGuid, Rev}).
