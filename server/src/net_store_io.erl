...
-import(netencode, [encode_linkmap/1, encode_list/1, encode_string/1,
...
do_read(Part, Offset, Length, #state{mps=MaxPS} = S) when Length =< MaxPS ->
	...
	net_store:io_request(Store, ?READ_REQ, ReqBody);
...
do_write(Part, Offset, Data, #state{mps=MaxPS} = S) when size(Data) =< MaxPS ->
	...
	case net_store:io_request(Store, ?WRITE_REQ, ReqBody) of
		{ok, <<>>} -> ok;
		Error      -> Error
	end;
...
relay_request(Request, Body, #state{handle=Handle, store=Store}) ->
	net_store:io_request(Store, Request, <<Handle:32, Body/binary>>).
