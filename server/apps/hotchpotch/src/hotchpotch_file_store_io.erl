...
-module(hotchpotch_file_store_io).
...
close_and_writeback(#state{parts=[{Part, Content} | RemParts]} = S) ->
	case Content of
		{rw, {FileName, IoDev}} ->
			case hotchpotch_util:hash_file(IoDev) of
				{ok, PId} ->
					file:close(IoDev),
					S2 = lock_part(PId, S),
					case hotchpotch_file_store:part_put(S2#state.store, PId, FileName) of
						ok ->
		...
		{rw, Data} when is_binary(Data) ->
			PId = binary_part(crypto:sha(Data), 0, 16),
			S2 = lock_part(PId, S),
			case hotchpotch_file_store:part_put(S#state.store, PId, Data) of
				ok ->
					S3 = commit_part(Part, PId, S2#state{parts=RemParts}),
					close_and_writeback(S3);
				{error, _Reason} = Error ->
...
lock_part(PId, #state{locks=Locks} = S) ->
	hotchpotch_file_store:part_lock(S#state.store, PId),
...
part_open_read(Part, #state{rev=Rev} = S) ->
	case lists:keyfind(Part, 1, Rev#revision.parts) of
		{Part, PId} ->
			case hotchpotch_file_store:part_get(S#state.store, PId) of
...
tmp_name(#state{store=Store}) ->
	hotchpotch_file_store:tmp_name(Store).
