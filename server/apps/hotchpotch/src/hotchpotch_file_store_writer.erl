...
-module(hotchpotch_file_store_writer).
...
handle_call({commit, Mtime}, _From, S) ->
	do_commit(fun hotchpotch_file_store:commit/4, S, Mtime);

% returns `{ok, Hash} | {error, Reason}'
handle_call({suspend, Mtime}, _From, S) ->
	do_commit(fun hotchpotch_file_store:suspend/4, S, Mtime);
...
terminate(_Reason, #ws{doc=Doc, locks=Locks, server=Server} = S) ->
	% unlock hashes
	lists:foreach(fun(Lock) -> hotchpotch_file_store:unlock(Server, Lock) end, Locks),
	% expose document to garbage collector
	hotchpotch_file_store:unhide(Server, Doc),
...
open_file_read(S, Part) ->
	case dict:find(Part, S#ws.orig) of
		{ok, Hash} ->
			FileName = hotchpotch_util:build_path(S#ws.path, Hash),
...
get_file_write(S, Part) ->
	case dict:find(Part, S#ws.new) of
		...
		error ->
			FileName = hotchpotch_util:gen_tmp_name(S#ws.path),
			{S2, FileHandle} = case dict:find(Part, S#ws.orig) of
				% part already exists; copy and open...
				{ok, Hash} ->
					case file:copy(hotchpotch_util:build_path(S#ws.path, Hash), FileName) of
						{ok, _} ->
...
close_and_move(S) ->
	NewParts = dict:fold(
		...
		fun (Part, {TmpName, IODevice}, Acc) ->
			{ok, Hash} = hotchpotch_util:hash_file(IODevice),
			file:close(IODevice),
			hotchpotch_file_store:lock(S#ws.server, Hash),
			NewName = hotchpotch_util:build_path(S#ws.path, Hash),
