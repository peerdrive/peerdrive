...
-module(hotchpotch_file_store_importer).
...
start_link(Store, Path, Rev, Revision, PartsDone, PartsNeeded, User) ->
	...
	NeededDict = lists:foldl(
		fun({FourCC, Hash}, Acc) ->
			FileName = hotchpotch_util:gen_tmp_name(Path),
		...
		ok ->
			dict:fold(
				fun (_Part, {Hash, TmpName, IODevice, _Md5Ctx}, _Acc) ->
					file:close(IODevice),
					NewName = hotchpotch_util:build_path(S#state.path, Hash),
				...
				ok,
				S#state.needed),
			hotchpotch_file_store:insert_rev(S#state.storepid, S#state.rev,
				S#state.revision);
