...
-module(hotchpotch_file_store_imp).
...
terminate(_Reason, #state{store=Store, parts=Parts, rev=Rev}) ->
	lists:foreach(
		fun({_, PId}) -> hotchpotch_file_store:part_unlock(Store, PId) end,
		Rev#revision.parts),
	lists:foreach(
...
do_commit(#state{store=Store, rid=RId, rev=Rev, parts=Parts} = S) ->
	try
		...
		case commit_parts(Store, Parts) of
			ok ->
				{
					hotchpotch_file_store:put_rev_commit(Store, RId, Rev),
					S#state{parts=[]}
				};
			{Error, Remaining} ->
...
commit_parts(Store, [{_FCC, {PId, Data, _}} | Rest]) when is_binary(Data) ->
	case hotchpotch_file_store:part_put(Store, PId, Data) of
		ok ->
...
commit_parts(Store, [{_FCC, {PId, {FileName, IoDev}, _}} | Rest]) ->
	file:close(IoDev),
	case hotchpotch_file_store:part_put(Store, PId, FileName) of
		ok ->
			commit_parts(Store, Rest);
...
create_tmp_file(Data, #state{store=Store}) ->
	TmpName = hotchpotch_file_store:tmp_name(Store),
