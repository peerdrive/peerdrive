...
-module(peerdrive_net_store_put).
...
do_commit(#state{handle=Handle} = S) ->
	Req = peerdrive_netstore_pb:encode_putdoccommitreq(
		#putdoccommitreq{handle=Handle}),
	relay_request(?PUT_DOC_COMMIT_MSG, Req, S).


do_close(#state{handle=Handle} = S) ->
	Req = peerdrive_netstore_pb:encode_putdocclosereq(
		#putdocclosereq{handle=Handle}),
	relay_request(?PUT_DOC_CLOSE_MSG, Req, S).
