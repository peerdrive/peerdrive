...
starting(publish, #state{info=Info, actual=Actual} = State) ->
	Tag = peerdrive_work_monitor:started(Info),
	if
		Actual /= 0 -> peerdrive_work_monitor:progress(Tag, Actual);
		true        -> ok
	end,
...
busy(stop, #state{tag=Tag} = State) ->
	peerdrive_work_monitor:done(Tag),
	{next_state, idle, State};
...
busy({progress, Progress}, #state{tag=Tag, informed=Informed} = State) ->
	if
		Progress =/= Informed ->
			...
			peerdrive_work_monitor:progress(Tag, Progress),
...
deaf(reactivate, #state{tag=Tag, actual=Actual, informed=Informed} = State) ->
	if
		Actual =/= Informed ->
			peerdrive_work_monitor:progress(Tag, Actual),
			Timer = gen_fsm:send_event_after(?UPDATE, reactivate),
